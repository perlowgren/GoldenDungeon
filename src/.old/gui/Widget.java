package com.seshat.gui;

import com.badlogic.gdx.Gdx;

import com.seshat.Rect;
import com.seshat.Size;
import com.seshat.gd.Data;
import com.seshat.gd.BasicScreen;

public class Widget extends Rect {
	private static final String className = "Widget";
	...
	public final void setActive(boolean b) { if(b) state |= ACTIVE;else if(!b) state &= ~ACTIVE; }
	public final boolean isActive() { return (state&ACTIVE)!=0; }

	public final void setFocus(boolean b) { if(b) state |= FOCUS;else if(!b) state &= ~FOCUS; }
	...
	public final void setDisabled(boolean b) { if(b) state |= DISABLED;else if(!b) state &= ~DISABLED; }
	public final boolean isDisabled() { return (state&DISABLED)!=0; }

	public final void setVirtual(boolean b) { if(b) state |= VIRTUAL;else if(!b) state &= ~VIRTUAL; }
	public final boolean isVirtual() { return (state&VIRTUAL)!=0; }

	public final void setClip(boolean b) { if(b) state |= CLIP;else if(!b) state &= ~CLIP; }
	public final boolean hasClip() { return (state&CLIP)!=0; }
...
Gdx.app.log(Data.APP_NAME,className+":getWidget(1: [x: "+w.x+", y: "+w.y+", w: "+w.width+", h: "+w.height+"], x: "+x+", y: "+y+")");
			if((w.state&(HIDDEN|DISABLED|VIRTUAL))==0 && w.contains(x,y)) {
Gdx.app.log(Data.APP_NAME,className+":getWidget(1: [x: "+w.x+", y: "+w.y+", w: "+w.width+", h: "+w.height+"], match!)");
				return w;
			}
			if(w.prev!=null) w = w.prev;
			else if(w.parent!=null) {
				for(w=w.parent; w!=null && w.prev==null; w=w.parent) {
Gdx.app.log(Data.APP_NAME,className+":getWidget(2: [x: "+w.x+", y: "+w.y+", w: "+w.width+", h: "+w.height+"], x: "+x+", y: "+y+")");
					if((w.state&(HIDDEN|DISABLED|VIRTUAL))==0 && w.contains(x,y)) {
Gdx.app.log(Data.APP_NAME,className+":getWidget(2: [x: "+w.x+", y: "+w.y+", w: "+w.width+", h: "+w.height+"], match!)");
						return w;
					}
...
Gdx.app.log(Data.APP_NAME,className+":calculateMinimumSize("+id+")");
...
Gdx.app.log(Data.APP_NAME,className+":arrange("+id+", width: "+width+", height: "+height+", minWidth: "+minWidth+", minHeight: "+minHeight+")");
		if((state&ARRANGE_H)!=0) {
			for(w=first,nx=0,ny=0; w!=null; w=w.next) {
				if((w.state&EXPAND)!=0) {
					i = (w.next!=null? w.minWidth+(nw/n) : width-nx);
Gdx.app.log(Data.APP_NAME,className+":arrange("+w.id+", ARRANGE_H - i: "+i+")");
					if((w.state&FILL)!=0) {
						w.x = x+nx;
						w.width = i;
					} else {
...
Gdx.app.log(Data.APP_NAME,className+":arrange("+w.id+", ARRANGE_V - i: "+i+")");
					if((w.state&FILL)!=0) {
						w.y = y+ny;
						w.height = i;
					} else {
