...
public class AnimationSet extends Rect implements Sprite {
	public Clip[] frames;
	public Clip frame;
	...
	public boolean isVisible(Size view,int x,int y) {
		return isVisible(view,frame,x,y);
	}

	public boolean isVisible(Size view,Clip f,int x,int y) {
		if(f==null) f = frame;
		return f!=null &&
		       x-f.centerX+f.width>0 &&
		       y-f.centerY+f.height>0 &&
		       x-f.centerX<view.width &&
		       y-f.centerY<view.height;
	}

	@Override
	public boolean draw(Size view,SpriteBatch batch,int x,int y) {
		if(!isVisible(view,frame,x,y)) return false;
	...
	public boolean draw(Size view,SpriteBatch batch,int x,int y,int i) {
		if(i<0) {
			i = frames.length+i;
		...
		Clip f = frames[i];
		if(!isVisible(view,f,x,y)) return false;
